...
public class MyDbTask extends DbTask {
    ...
    protected ArrayList<EmailSendFrom> getEmail(String typeEmail) throws Exception {
        ...
        String hour_run = ipSplit[2];
        String day_run = ipSplit[3];
        try {
...
//            sql.append("from common_email a where a.is_send_email =0 and a.sender_email_code =?");
            sql.append("select a.email_id, a.sender_email_code, a.content, a.is_file,a.subject, a.log_server_id,"
                    + "  a.path, a.file_name, a.is_send_email,a.error_msg,b.email,b.hour_run,b.day_run from common_email a,cata_email b "
                    + " where a.type_report=b.type_report and a.type=b.type and b.type=2 and a.is_send_email =0 "
                    + " and a.sender_email_code =? and b.hour_run=? and b.day_run=? and b.email =?");
            pstmt = con.prepareStatement(sql.toString());
            pstmt.setString(1, fuction_code);
            pstmt.setString(2, hour_run);
            pstmt.setString(3, day_run);
            ...
            while (rs.next()) {
                ...
                emailBo.setDay_run(DBUtil.getInteger(rs.getObject("day_run")));
                //hoanm1_add_end
                lst.add(emailBo);
            }
    ...
    protected List<String> getlstMail(EmailSendFrom emailSendFrom) throws Exception {
        ...
        try {
            ...
            con = getConnectionPool();
            StringBuilder sql = new StringBuilder();
            sql.append("select a.email from cata_email a,common_map_email b where a.type_report=b.type_report and "
                    + " a.type=b.type and b.type=2 and b.function_code=? and a.hour_run=? and a.day_run=? and a.email=?");
            pstmt = con.prepareStatement(sql.toString());
            pstmt.setString(1, emailSendFrom.getSenderEmailCode());
            pstmt.setLong(2, emailSendFrom.getHour_run());
            pstmt.setLong(3, emailSendFrom.getDay_run());
            pstmt.setString(4, emailSendFrom.getEmail());
            rs = pstmt.executeQuery();
            while (rs.next()) {
