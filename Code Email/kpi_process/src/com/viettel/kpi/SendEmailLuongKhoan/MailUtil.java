...
import com.viettel.kpi.SendEmailLuongKhoan.*;
...
import com.viettel.kpi.SendEmailLuongKhoan.MscServer;
...
public class MailUtil {
    ...
    public boolean sendMail(EmailSendFrom emailSendFrom, MyDbTask db) {
        boolean checkSendMail;
        try {
            InputStream input = new FileInputStream("../conf/program.conf");
            PropertyResourceBundle bunder = new PropertyResourceBundle(input);
...
//                hoanm1_20180228_end
            ...
            checkSendMail = postMail(emailSendFrom, fromEmail, smtpHostName, db, dir);
        } catch (Exception ex) {
            checkSendMail = false;
            logger.info("Gui mail bi loi");
            logger.error(ex.getMessage(), ex);
...
//        return checkSendMail;
    ...
    public boolean postMail(EmailSendFrom emailSendFrom,
        ...
        try {
            ...
            subject = "Thông báo quyết toán lương tháng " + thang;
            ...
            String errorEmail = "";
            try {
                addressTo[i] = new InternetAddress(emailSendFrom.getEmail());
                msg.setRecipients(Message.RecipientType.TO, addressTo[i].getAddress());
                Multipart multipart = new MimeMultipart();

                if (isFile == 1) {
                    DataSource source;
                    String[] account = emailSendFrom.getEmail().split("@");
                    File fileExport = new File(dir + account[0] + "_luong thang " + thang + ".xlsx");
                    ...
                    content = "Kính gửi đồng chí " + account[0] + " lương tháng " + thang + " Khối cơ quan và 63 TTKT!!!";
