...
//    DbTask db;
    ...
    public ProccessThreadService(Long interval,Long hour) throws Exception {
    ...
    protected void process() {
        try {
            ...
            client = new MyClient();
                ...
                if (hour == hourCurrent) {
                    ProccessEmailThreadService thread = new ProccessEmailThreadService(logger);
                    thread.run();
                }
            ...
            try {
                if (client != null) {
                    client.close();
                    logger.info("Đã chạy xong tiến trình");
                }
            } catch (Exception ex) {
                logger.error(ex.getMessage(), ex);
            }
