 ...
 */
public class ServiceManager extends ZkProcess {
...
//    }
    ...
    /**
     * Hàm start tiến trình. Khởi tạo các Thread của tiến trình. Mỗi Thread xử
     * lý với 1 LogServer.
     *
...
//                        process.start();
    ...
    /**
     * Hàm start tiến trình chạy theo giờ (interval = 1 hour). Khởi tạo các
     * Thread của tiến trình. Mỗi Thread xử lý với 1 LogServer.
     *
     * @param serverList Danh sách LogServer
     * @param threadSize Số lượng LogServer tối đa mà 1 Thread xử lý
     * @param job Class mô tả công việc của tiến trình
     ...
     */
    public static void startHourly(ArrayList<LogServer> serverList, Job job, long minute) {
        try {
            if (serverList != null && serverList.size() > 0) {
                for (LogServer logServer : serverList) {
                    ProcessThreadHourly process = new ProcessThreadHourly(logServer, job, minute);
                    processList.add(process);
//                    process.start();
                }
                //HA_start
    ...
    /**
     * Hàm start tiến trình chạy theo ngày (interval = 1 day). Khởi tạo các
     * Thread của tiến trình. Mỗi Thread xử lý với 1 LogServer.
     *
     * @param serverList Danh sách LogServer
     * @param threadSize Số lượng LogServer tối đa mà 1 Thread xử lý
...
//                    process.start();
     ...
     * @param jobList Class mô tả công việc của từng tiến trình
     * @param hour giờ trong ngày mà tiến trình thực hiện lấy dữ liệu
     * @param minute phút tiến trình chạy trong giờ đó Đã thêm giám sát M&M
     * server: process:*
     */
...
//                    process.start();
    ...
    protected void doStart() {
        for (ProcessThreadMX process : processList) {
            try {
                process.start();
            } catch (Exception e) {
                e.printStackTrace();
