...
import com.viettel.kpi.client.ftp.SFtpClient;
 ...
 */
public class MySFtpClient extends SFtpClient {
    ...
    public synchronized int parse(Map<CommonSvFtpColumnMapPK, CommonSvFtpColumnMap> columnMap,
            ...
            List<String> listClientColumnName, CommonMapQueryLogServer mapQueryServer, List<String> listClientColNameIsNotNull, boolean checkRaw) throws Exception {

        logger.info("----------------------------------------");
        logger.info("File ID    : " + svFtpFile.getFileId() + "; File Name  : " + svFtpFile.getRemoteFileName());
        logger.info("LogServer ID    : " + commonLogServer.getLogServerId());
        logger.info("Table Name : " + svFtpFile.getTableName());
        logger.info("Start Time : " + startTime);
        logger.info("----------------------------------------");

        //B1: getFile from ftp
        Map<Timestamp, List<Map<String, InputStream>>> listIs = getFile(svFtpFile, startTime, endTime, logger);
        logger.info("LIST:" + listIs.size());
...
//                logger.error("Khong lay duoc file tu server FTP");
     ...
     * danh sach nay
     */
    private Map<Timestamp, List<Map<String, InputStream>>> getFile(CommonSvFtpFile svFtpFile, Timestamp startTime, Timestamp endTime, Logger logger) {
            ...
            if (period > 0) {
                ...
                if (svFtpFile.getDataLevel() == 1) {
                    ...
                    for (int i = 0; i < period; i++) {
                        ...
                        inputStream = getFileStream(folder, fileName, logger, localStore, timestamp, svFtpFile.getLocalPathFormat());
