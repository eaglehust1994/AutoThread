...
public class ProccessEmailThreadService implements Runnable {
    ...
    public void run() {
        ...
        String content = lst.get(0).getBodyEmail();
        String subject = lst.get(0).getSubject();
        String path = lst.get(0).getPathFile();
        String logServerId = lst.get(0).getLogServerId();
        String functionCode = lst.get(0).getFunctionCode();
        String formatContent = lst.get(0).getFomatBodyEmail();
        String fomatSubject = lst.get(0).getFomatSubject();
        String formatPath = lst.get(0).getFomatPathFile();
        ...
        if (formatContent != null && !formatContent.equals("")) {
            String _timeContent = DateTimeUtils.convertDateTimeToString(time, formatContent);
            content = content.replaceAll(formatContent, _timeContent);
        }
        if (formatPath != null && !formatPath.equals("")) {
            String _timePath = DateTimeUtils.convertDateTimeToString(time, formatPath);
            path = path.replaceAll(formatPath, _timePath);
        }
        try {
            String _fileName = "";
//            String senderEmail = client.getSenderEmail(lst.get(0).getFunctionCode());
            String senderEmail = lst.get(0).getFunctionCode() + "|" + lst.get(0).getEmail() + "|" + lst.get(0).getHour_run() + "|" + lst.get(0).getDay_run();
            ObjectEmail objEmail = new ObjectEmail();

            if (isFile == 1) {
                boolean checkExport = onExport(client, lst);
                if (checkExport) {
                    if (lstFileName != null && !lstFileName.isEmpty()) {
                        _fileName = lstFileName.get(0);
                        objEmail.setIsFile(1);
                        String _bodyHtml = "";
                    ...
                    if (fomatSubject != null && !fomatSubject.equals("")) {
                        ...
                        String _timeSubject = +month + 1 + "_" + year;
            ...
            if (!senderEmail.equals("")) {
                // update
                objEmail.setSenderEmailCode(functionCode);
                objEmail.setFileName(_fileName);
                objEmail.setPath(path);
                objEmail.setLogServerId(logServerId);
                objEmail.setContent(content);
                objEmail.setSubject(subject);

                client.updateCommonEmail(objEmail, logger);
                senderEmail = PassProtector.encrypt(senderEmail, "KPI");
                try {
                ...
                objEmail.setContent(content);
                objEmail.setSubject(subject);
                client.insertDataWsToDb(objEmail, logger);
                senderEmail = PassProtector.encrypt(senderEmail, "KPI");
                try {
                    sendMessage(senderEmail);
                } catch (Exception ex) {
            ...
            // Set lai truong end_time_run trong bang common_map_email ve thoi gian hien tai
            client.updateCommonMapEmail(functionCode, lst, logger);
        } catch (Exception ex) {
            logger.error(ex.getMessage(), ex);
        } finally {
...
//            if (isFile == 1) {
            ...
            if (lstFileName != null && !lstFileName.isEmpty()) {
                ...
                int month = cal.get(Calendar.MONTH);
                int year = cal.get(Calendar.YEAR);
                String thang = +month + 1 + "_" + year;
                String reportName = lstFileName.get(0) + "_" + thang + ".xlsx";
                //muc ngay
//                Date timeWeek = new Date();
...
//                String reportName = lstFileName.get(0) + "_" + updateTime + ".xlsx";
                ...
                //dungvv8_28112015 khong dung ftp
//                String logSever = lst.get(0).getLogServerId();
//                File reportFile = new File(pathOut); //Lấy ra file báo cáo đã xuất ra ở trên hoặc file template
//                MscServer severFtp = client.getLogServerID(logSever, logger);
//                String password = PassProtector.decrypt(severFtp.getPassword(), severFtp.getIp());
//                ConnectionFTP.getInputStream(severFtp.getIp(), 21, severFtp.getUserName(),
//                        password, path, reportFile);
            }
//            }
        } catch (Exception ex) {
            logger.error(ex.getMessage(), ex);
...
//                    time.setDate(time.getDate() - 1);
                    ...
                    if (exportSheet == null) {
                        ...
                        int month = cal.get(Calendar.MONTH);
                        int year = cal.get(Calendar.YEAR);
                        String thang = +month + 1 + "-" + year;
                        excelUtil.createCellObject(exportSheet, 1, 5, "Tháng báo cáo:" + thang, csSubTitle);
...
//                            headerInfors.add(new Object[]{getFormatColumn(obj.getColumnName() + ": " + obj.getColumnNameL1(), time, dateFormat, 1), 7000});
                        ...
                        if (StringUtils.isNotNull(obj.getColumnNameL1())) {
                            ...
                            String thang = "";
                            String NamLevel1 = "";
                            if (obj.getColumnNameL1().equals("Thang n-4")) {
                                thang = +month - 2 + "/" + year;
                                NamLevel1 = obj.getColumnNameL1().replace(obj.getColumnNameL1(), "Trước tháng " + thang);
                            } else if (obj.getColumnNameL1().equals("Thang n-3")) {
                                thang = +month - 2 + "/" + year;
                                NamLevel1 = obj.getColumnNameL1().replace(obj.getColumnNameL1(), "Tháng " + thang);
                            } else if (obj.getColumnNameL1().equals("Thang n-2")) {
                                thang = +month - 1 + "/" + year;
                                NamLevel1 = obj.getColumnNameL1().replace(obj.getColumnNameL1(), "Tháng " + thang);
                            } else if (obj.getColumnNameL1().equals("Thang n-1")) {
                                thang = +month + "/" + year;
                                NamLevel1 = obj.getColumnNameL1().replace(obj.getColumnNameL1(), "Tháng " + thang);
                            } else if (obj.getColumnNameL1().equals("Thang n")) {
                                thang = +month + 1 + "/" + year;
                                NamLevel1 = obj.getColumnNameL1().replace(obj.getColumnNameL1(), "Tháng " + thang);
                            } else {
                                NamLevel1 = obj.getColumnNameL1();
                            }
//                            columnNameLevel1.add(obj.getColumnNameL1());
                            columnNameLevel1.add(NamLevel1);
                        } else {
...
//                    mapRowHeader.put(sheetName, rowHeader + row - 8 + 4);
    ...
    public String getFormatColumn(String columnName, Date reportDate, SimpleDateFormat dateFormat, int type) {
        if (reportDate != null) {
            ...
            for (int j = 20; j >= 0; j--) {
                if (columnName.contains("Z2+Z1+Z0")) {
//                    date.setMonth(date.getMonth() - j);
                    return columnName.replace("Z2+Z1+Z0", "T" + (date.getMonth() - 1) + "+" + "T" + (date.getMonth()) + "+"
                            + "T" + (date.getMonth() + 1));
                }
            }
        }
